...
void ConnectTier2Libraries( CreateInterfaceFn* pFactoryList, int nFactoryCount )
{
    ...
    for ( int i = 0; i < nFactoryCount; ++i )
    {
        ...
        {
            g_pFullFileSystem = (IFileSystem*)pFactoryList[i](
                FILESYSTEM_INTERFACE_VERSION, NULL );
        }
        if ( !materials )
        {
        ...
        if ( !g_pInputSystem )
        {
            g_pInputSystem = (IInputSystem*)pFactoryList[i](
                INPUTSYSTEM_INTERFACE_VERSION, NULL );
        }
        if ( !g_pNetworkSystem )
        {
        ...
        if ( !g_pMaterialSystemDebugTextureInfo )
        {
            g_pMaterialSystemDebugTextureInfo =
                (IDebugTextureInfo*)pFactoryList[i]( DEBUG_TEXTURE_INFO_VERSION,
                                                     0 );
        }
        if ( !g_VBAllocTracker )
        {
        ...
        {
            mdllib =
                (IMdlLib*)pFactoryList[i]( MDLLIB_INTERFACE_VERSION, NULL );
        }
        /*if ( !g_pQueuedLoader )
        {
