...
// constructor, destructor
//-----------------------------------------------------------------------------
CAppSystemGroup::CAppSystemGroup( CAppSystemGroup* pAppSystemParent )
...
AppModule_t CAppSystemGroup::LoadModule( const char* pDLLName )
{
    ...
    if ( !pSysModule )
    {
        Error( "AppFramework : Unable to load module %s!\n", pDLLName );
    ...
    GetCSO2LoadingSplash()->SetLoadingProgress( ( nIndex - 2 ) / 2 * 5 );
    GetCSO2LoadingSplash()->Unknown00();
...
CSysModule* CAppSystemGroup::GetSysModuleFromIndex( AppModule_t moduleIndex )
{
    if ( !m_Modules.IsValidIndex( moduleIndex ) )
        return nullptr;

    return m_Modules[moduleIndex].m_pModule;
}
                                        ...
                                        const char* pInterfaceName )
{
    ...
    {
        Error( "AppFramework : Unable to create system %s!\n", pInterfaceName );
        return NULL;
    }
...
bool CAppSystemGroup::AddSystems( AppSystemInfo_t* pSystemList )
{
    while ( pSystemList->m_pModuleName[0] )
    {
        ...
        if ( !pSystem )
        {
            Error( "Unable to load interface %s from %s\n",
...
destroy:
    ...
    // Have to do this because the spew func may point to a module which is
    // being unloaded
                                             ...
                                             bool bIsTool )
{
    ...
    Q_strncpy( m_pGameInfoPath, steamInfo.m_GameInfoPath,
               sizeof( m_pGameInfoPath ) );
    return true;
}
