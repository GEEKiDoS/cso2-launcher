...
void V_StripExtension( const wchar_t *in, wchar_t *out, int outSize )
{
	// Find the last dot. If it's followed by a dot or a slash, then it's part of a 
	// directory specifier like ../../somedir/./blah.

	// scan backward for '.'
	int end = V_wcslen( in ) - 1;
	while ( end > 0 && in[end] != L'.' && !PATHSEPARATOR( in[end] ) )
	{
		--end;
	}
	...
	if ( end > 0 && !PATHSEPARATOR( in[end] ) && end < outSize )
	{
		...
		if ( out != in )
		{
			memcpy( out, in, nChars * sizeof( wchar_t ) );
		}
		out[nChars] = 0;
	}
	else
...
void  V_StripFilename( wchar_t *path )
{
	int             length;

	length = V_wcslen( path ) - 1;
	if ( length <= 0 )
		return;
...
const wchar_t * V_GetFileExtension( const wchar_t * path )
{
	const wchar_t    *src;

	src = path + wcslen( path ) - 1;
