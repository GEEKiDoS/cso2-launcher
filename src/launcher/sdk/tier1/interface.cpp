...
void RaiseLoadLibraryError( bool bFoundLibrary, const char* szModuleName )
{
	...
	FormatMessageA( FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER, nullptr, dwLastError, NULL, (char*) &pMessage, NULL, nullptr );
	V_snprintf( szMessageBuffer, sizeof( szMessageBuffer ), "IntLoadLib Failed(%d)\n%d:%s (%s)", bFoundLibrary, dwLastError, pMessage, szModuleName );
	LocalFree( pMessage );
	MessageBoxA( NULL, szMessageBuffer, "error", MB_ICONERROR );
	COM_WriteReportLog( 1, szMessageBuffer );
	RaiseException( 0xE000001C, NULL, NULL, nullptr );
}

static HMODULE InternalLoadLibrary( const char *pName, Sys_Flags flags )
{
	using fnInit_t = HMODULE( *)(const char*, Sys_Flags);

	HMODULE hMainModule = GetModuleHandleA( nullptr );

	if ( !hMainModule )
	{
		RaiseLoadLibraryError( false, pName );
		return nullptr;
	}

	fnInit_t pfnInit = (fnInit_t) GetProcAddress( hMainModule, "_Init" );
	...
	HMODULE hResult = pfnInit( pName, flags );

	if ( !hResult )
	{
		if ( strcmp( pName, "shaderapiempty.dll" ) && strcmp( pName, "shadercompile_dll.dll" ) )
			RaiseLoadLibraryError( true, pName );
	}
...
CSysModule *Sys_LoadModule( const char *pModuleName, Sys_Flags flags /* = SYS_NOFLAGS (0) */ )
{
	...
	{			  
		...
		Q_snprintf( szAbsoluteModuleName, sizeof(szAbsoluteModuleName), "%s", modulePath.string().c_str() );

		Log( "LoadDLL Path info %s", szAbsoluteModuleName );
