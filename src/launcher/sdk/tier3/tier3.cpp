...
IVideoServices* g_pVideoServices = 0;
...
void ConnectTier3Libraries( CreateInterfaceFn* pFactoryList, int nFactoryCount )
{
    ...
    Assert( !g_pStudioRender && !studiorender && !g_pMatSystemSurface &&
            ...
            !g_pDataCache && !g_pMDLCache && !mdlcache && !g_pVideoServices &&
    ...
    for ( int i = 0; i < nFactoryCount; ++i )
    {
        ...
        if ( !g_pVGui )
        {
            g_pVGui = (vgui::IVGui*)pFactoryList[i](
                VGUI_IVGUI_INTERFACE_VERSION, NULL );
        }
        if ( !g_pVGuiInput )
        {
            g_pVGuiInput = (vgui::IInput*)pFactoryList[i](
                VGUI_INPUT_INTERFACE_VERSION, NULL );
        }
        if ( !g_pVGuiPanel )
        {
            g_pVGuiPanel = (vgui::IPanel*)pFactoryList[i](
                VGUI_PANEL_INTERFACE_VERSION, NULL );
        }
        if ( !g_pVGuiSurface )
        {
        ...
        if ( !g_pVGuiSystem )
        {
            g_pVGuiSystem = (vgui::ISystem*)pFactoryList[i](
                VGUI_SYSTEM_INTERFACE_VERSION, NULL );
        }
        if ( !g_pVGuiLocalize )
        {
        ...
        if ( !g_pDataCache )
        {
            g_pDataCache = (IDataCache*)pFactoryList[i](
                DATACACHE_INTERFACE_VERSION, NULL );
        }
        if ( !g_pMDLCache )
        {
        ...
        if ( !g_pVideoServices )
        {
            g_pVideoServices = (IVideoServices*)pFactoryList[i](
                VIDEO_SERVICES_INTERFACE_VERSION, NULL );
        }
